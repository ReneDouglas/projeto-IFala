...
const TLD_VALIDAS = [
  ...
  'br', 'com.br', 'edu.br', 'gov.br', 'org.br', 'net.br', 'mil.br',
  'io', 'me', 'tech', 'dev', 'app', 'online', 'site', 'website',
  'uk', 'us', 'de', 'fr', 'it', 'es', 'pt', 'ar', 'mx', 'cl', 'co.uk',
];
...
export const validateEmail = (email: string): string => {
  ...
  const tldWith2Parts = domainParts.length >= 2 
  ...
  if (lowerEmail.includes('@ifpi.edu.br') || lowerEmail.includes('@aluno.ifpi.edu.br')) {
    ...
    else if (lowerEmail.includes('ifpi') && !lowerEmail.endsWith('@ifpi.edu.br') && !lowerEmail.endsWith('@aluno.ifpi.edu.br')) {
  ...
  const isDominioComum = DOMINIOS_COMUNS.some(d => 
    dominioCompleto === d || dominioCompleto.endsWith('.' + d)
  );
  
  const isTLDValida = TLD_VALIDAS.includes(tld) || TLD_VALIDAS.includes(tldWith2Parts);
...
export const getEmailSuggestion = (email: string): string => {
  ...
  if (trimmedEmail.includes('ifpi')) {
    ...
    if (/\d{6,}/.test(trimmedEmail)) {
      ...
      if (!trimmedEmail.includes('@ifpi.edu.br') && !trimmedEmail.includes('@aluno.ifpi.edu.br')) {
        return 'Sugestão: Servidores/professores devem usar @ifpi.edu.br';
      }
    }
    
    if (trimmedEmail.includes('@ifpi.br') || 
